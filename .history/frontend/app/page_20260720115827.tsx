...
const mockMetrics: DashboardMetrics = {
  ...
  total_cost: 2.34,
  avg_latency_ms: 340,
  tool_usage: { github: 120, browser: 20, pdf_reader: 50, gmail: 45, slack: 30, gdrive: 25 },
  model_usage: { ollama: 800, claude: 200, gpt: 150, gemini: 97 },
  cost_by_model: { ollama: 0, claude: 1.25, gpt: 0.75, gemini: 0.34 },
  latency_by_tool: { github: 150, browser: 230, pdf_reader: 300, gmail: 180, slack: 120, gdrive: 200 },
};
...
const COLORS = ["#E57035", "#10A37F", "#4285F4", "#FF6B6B", "#8B5CF6", "#F59E0B"];
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `I processed your request: "${input}"\n\n🔍 Discovered tools: github, browser\n🛡️  Permission check: passed\n⚡ Executed in sandbox\n\nResult: Here's what I found based on your query. In a real deployment, this would execute actual MCP tools.`,
            model: `${selectedModel === "ollama" ? "Ollama/llama3.1:8b" : selectedModel}`,
            tools: [{ tool_name: "github", success: true, latency_ms: 150 }],
            cost: selectedModel === "ollama" ? 0 : 0.05,
            latency: 340,
          },
        ]);
      }
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "I'm running in demo mode. The backend appears to be offline, but this shows the UI structure. Start the backend with `docker-compose up` to see full functionality.",
          model: "demo",
        },
      ]);
    } finally {
              ...
              <div className="border-t border-slate-800 p-4">
                <div className="max-w-3xl mx-auto flex gap-3">
                  <input
                    ...
                    placeholder="Ask OpenMesh AI anything... (e.g., 'Find me tools to read PDFs' or 'Run job application workflow')"
                ...
                {[
                  {
                    ...
                    description: "Resume → Find Jobs → Tailor → Send → Track",
                    steps: 7,
                    icon: FileText,
                  ...
                  {
                    name: "Code Review",
                    description: "PR → Analyze → Review → Comment",
                    steps: 4,
                    icon: GitBranch,
                  ...
                  },
                  {
                    ...
                    description: "Topic → Search → Synthesize → Report",
                    steps: 4,
                    icon: Globe,
                  ...
                  <div
                    key={workflow.name}
                    className="group relative p-6 rounded-2xl bg-slate-800/50 border border-slate-700 hover:border-slate-600 transition-all cursor-pointer hover:bg-slate-800"
                  >
                    <div
                      className={`w-12 h-12 rounded-xl bg-gradient-to-br ${workflow.color} flex items-center justify-center mb-4`}
                    >
                    ...
                    <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-600 group-hover:text-slate-400 transition-colors" />
                  </div>
                ))}
              </div>

              <div className="mt-8 p-6 rounded-2xl bg-slate-800/30 border border-dashed border-slate-700">
                <h3 className="font-semibold mb-2">Custom Workflow</h3>
                <p className="text-sm text-slate-400 mb-4">
                  Describe what you want to automate, and the Planner Agent will generate a workflow.
                </p>
                <div className="flex gap-3">
                  <input
                    type="text"
                    placeholder="e.g., 'Monitor GitHub repo for new issues and notify Slack'"
          ...
          {activeTab === "tools" && (
            <div className="p-6">
              ...
              <div className="mb-6">
                <input
                  type="text"
                  placeholder="Search tools by name, capability, or description..."
                  className="w-full max-w-md bg-slate-800 border border-slate-700 rounded-xl px-5 py-3 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
                      ...
                      {Object.entries(metrics.tool_usage).map(([tool, count]) => (
                        <tr key={tool} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                          ...
                          <td className="py-3 px-4">
                            {metrics.latency_by_tool[tool] || 0}ms
                          </td>
                          <td className="py-3 px-4">
